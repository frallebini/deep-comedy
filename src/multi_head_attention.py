...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    The scaled dot product attention is applied to each head (broadcasted for efficiency). An appropriate mask must be
    used in the attention step. The attention output for each head is then concatenated (using ``tf.transpose`` and
    ``tf.reshape``) and put through a final dense layer.

    Instead of one single attention head, q, k, and v are split into multiple heads because it allows the model to
    jointly attend to information at different positions from different representational spaces. After the split, each
